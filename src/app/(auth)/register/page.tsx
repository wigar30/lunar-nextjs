...
export default function Page() {
  ...
  const [animateDestroyToLogin, setAnimateDestroyToLogin] = useState(false)
  const [animateDestroyToResetPassword, setAnimateDestroyToResetPassword] = useState(false)
  ...
  const handleClickToResetPassword = () => {
    setAnimateDestroyToResetPassword(true)
  }
  const handleClickToLogin = () => {
    setAnimateDestroyToLogin(true)
  }
  const handleAnimationEnd = () => {
    if (animateDestroyToLogin) {
      router.push('/login')
    }

    if (animateDestroyToResetPassword) {
      ...
      <section className="w-full h-full flex flex-nowrap relative">
        <div
          className={clsx(
            ...
            [animateDestroyToLogin || animateDestroyToResetPassword ? 'w-full' : 'w-1/3'],
            'h-full transition-all delay-100 duration-300 bg-primary-200 dark:bg-primary-800 flex-none absolute z-50 left-0'
          )}
        ...
        <div className={clsx('w-2/3 h-full bg-primary-950 dark:bg-primary-50 transition-all flex flex-col items-start px-4 py-8')}>
          ...
          <Text className="text-sm pointer-events-none select-none" weight="normal">
            Gabung di marketplace untuk dapatkan beragam keuntungan
          </Text>

          <div className="w-2/3">
            <form className="mt-6 w-full px-6" method="post" onSubmit={handleSubmit}>
              <label htmlFor="nama" className="mt-2">
                <Text weight="semibold" className="mt-2">
                  Nama
                </Text>
                <Input name="nama" type="text" required placeholder="Nama" onInput={(e) => setForms({ ...forms, name: e.target.value })} />
              </label>
              ...
              <label htmlFor="password" className="mt-6">
                ...
                <div className="relative">
                  <Input name="password" type={showPassword ? 'text' : 'password'} required placeholder="Konfirmasi Password" onInput={(e) => setForms({ ...forms, password: e.target.value })} />
            ...
            <div className="w-full flex justify-center mt-6 space-x-1">
              ...
              <Text className="text-xs cursor-pointer" onClick={handleClickToLogin}>
                Login Sekarang
              </Text>
            </div>
          </div>
